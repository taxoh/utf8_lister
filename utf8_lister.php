...
if ($_POST)
{
	IntlChar::enumCharNames(0x0000, 0xffff, function($codepoint,$shit,$name)use(&$regex, &$res, $types, $categories){
		...
		if ((stripos($name, $_POST['search_for'])!==FALSE) || 
			...
			($_POST['use_cat'] && stripos($cat, $_POST['search_for'])!==FALSE) ||
...
<form action="?" method=post>
	...
	<input type=submit>
	<input type=checkbox name=use_cat id=use_cat value=1 <?=($_POST['use_cat']?'checked':'')?>>
	<label for=use_cat>в т.ч. по категориям</label>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	|
	найдено: <?=count($res)?>
</form>
